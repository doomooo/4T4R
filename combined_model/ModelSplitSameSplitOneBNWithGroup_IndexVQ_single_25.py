...
class Encoder(nn.Module):
    ...
    def __init__(self, bit_list = None):
        ...
        # 这个是根据之前训练的模型，统计的一个量
        # 统计没有量化之前Encoder的各个节点的方差（代表了信息量）
        # 然后把信息量最少的几个用2bit来量化，后面的用3bit来优化
        # 这个参数按理说应该是和模型相关的，但为了复现简单，就固定了，没做其他的对比实验；
        # 不过即使不看这个混合bit，也是第二的成绩，加了这个混个bit想冲击第一，没有冲击成功，就草草收场了；
        # 加了混合bit，代码上会增加一点点难度
...
class Decoder(nn.Module):
    ...
    def __init__(self,  bit_list = None):
        ...
        std_sort = [102, 103,  88,  96,  97,  89,  99,  90, 100,  91,  85,  83,  98,  81,
        101,  82,  79,  62,  94,  86,  92,  95,  80,  56,  54,  71,  70,  59,
         73,  76,  72,  58,  74,  29,  51,  93,  45,  47,  43,  67,  48,  40,
         38,  77,  68,  39,  60,  11,  34,  49,   2,  50,  36,  37,  17,  35,
         18,  25,  32,  24,  65,  23,  27,   8,   7,  14,  13,  22,  21,   9,
         42,  12,  41,  46,  16,  61,  31,  26,  10,  20,   5,   6,  30,   1,
         15,  28,   4,  57,  52,  53,  64,   0,   3,  75,  33,  19,  55,  66,
         84,  63,  69,  44,  87,  78] # origin
